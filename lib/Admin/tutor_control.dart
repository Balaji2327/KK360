...
class _TutorControlScreenState extends State<TutorControlScreen> {
  ...
  Future<void> _loadTutors() async {
    ...
    try {
      ...
      for (var tc in testCreators) {
        tc['role'] = 'Test Creator';
      }

      setState(() {
        _tutors = [...tutors, ...testCreators];
  ...
  Future<void> _importTutorsFromExcel() async {
    if (_isImportingTutors) {
      return;
    }

    setState(() => _isImportingTutors = true);
    ...
    try {
      ...
      if (sheet.rows.isEmpty) {
        throw 'The selected Excel file has no tutor rows.';
      }
      ...
      for (var index = 0; index < sheet.rows.length; index++) {
        ...
        final tutorId = _excelImportService.readValue(row, [
          'tutor_id',
          'tutorid',
          'id',
        ]);
        ...
        final role =
            _excelImportService.readValue(row, ['role'])?.toLowerCase() ??
            'tutor';

        if (tutorId == null || name == null || email == null || password == null) {
          failures.add(
            'Row $rowNumber: tutor_id, name, email, and password are required.',
          );
          continue;
        }

        if (password.length < 6) {
          failures.add('Row $rowNumber ($tutorId): password must be 6+ characters.');
          continue;
        }

        if (role != 'tutor' && role != 'test_creator') {
          failures.add(
            'Row $rowNumber ($tutorId): role must be tutor or test_creator.',
          );
          continue;
        }
        ...
        try {
          ...
          successes.add(tutorId);
        } catch (e) {
          failures.add('Row $rowNumber ($tutorId): $e');
        }
      }
      ...
      _showImportResultDialog(
        title: 'Tutor Import Result',
        successLabel: 'Tutors added',
        successes: successes,
        failures: failures,
        requiredColumns: 'tutor_id, name, email, password, role(optional)',
      );
    } catch (e) {
      if (!mounted) {
        return;
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Failed to import tutors: $e'),
          backgroundColor: Colors.red,
        ),
      );
    } finally {
      if (mounted) {
        setState(() => _isImportingTutors = false);
      }
    }
  }
  ...
  void _showAddTutorDialog(BuildContext context) {
    ...
    final GlobalKey<FormState> formKey = GlobalKey<FormState>();
    String selectedRole = 'tutor'; // Default role
