...
class TutorInviteStudentsScreen extends StatefulWidget {
  ...
  const TutorInviteStudentsScreen({super.key, this.initialClassId});
...
class _TutorInviteStudentsScreenState extends State<TutorInviteStudentsScreen> {
  ...
  void initState() {
    super.initState();
    debugPrint(
      '[InviteStudent] initState - initialClassId: ${widget.initialClassId}',
    );
    // Set the selected class ID immediately if provided
    if (widget.initialClassId != null && widget.initialClassId!.isNotEmpty) {
      _selectedClassId = widget.initialClassId;
      debugPrint('[InviteStudent] Set _selectedClassId to: $_selectedClassId');
    }
    _loadClasses();
  }
                ...
                Row(
                  ...
                  children: [
                    ...
                    GestureDetector(
                      onTap: () async {
                        debugPrint(
                          '[InviteStudent] Invite clicked - selectedClassId: $_selectedClassId, initialClassId: ${widget.initialClassId}',
                        );
                        ...
                        try {
                          debugPrint(
                            '[InviteStudent] Starting invite process for emails: $_emails',
                          );
                          debugPrint(
                          ...
                          debugPrint('[InviteStudent] Error during invite: $e');
                          debugPrint(
                            '[InviteStudent] Stack trace: $stackTrace',
                          );
                          if (!mounted) return;
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                  ...
                  if (widget.initialClassId == null && _classes.isNotEmpty)
                    Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          Text(
                            'Select a class to invite students to:',
                            style: TextStyle(
                              fontSize: 16,
                              fontWeight: FontWeight.w600,
                              color: purple,
                      ...
                      child: Container(
                        ...
                        child: Row(
                          children: [
                            ...
                            Expanded(
                              child: Text(
                                'You need to create a class first before inviting students.',
                                style: TextStyle(
                                  color: Colors.orange.shade700,
                                  fontSize: 14,
                                ),
